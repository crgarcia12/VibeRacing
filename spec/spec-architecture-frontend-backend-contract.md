...
This specification defines the current browser-facing contract between the VibeRacing frontend and backend. Its purpose is to let a different team, or a Generative AI system, build a fully compatible frontend without access to the backend source code.

This document is intentionally self-contained. A student or implementation team should be able to build a compatible frontend from this specification alone, without reading any C#, TypeScript, or repository internals. It is intentionally precise about transport, message names, payloads, sequencing, and client obligations. It is intentionally flexible about visual style so independent frontends do not converge on a single look.
...
- Intended audience:
  ...
  - students who are given only this specification and the backend URL
- In scope:
  - HTTP endpoints
...
- **GUD-006**: This specification SHOULD be treated as the complete student-facing contract. A compatible frontend SHOULD NOT require backend source access to be implemented.
...
- **Test Levels**:
  - Contract unit tests for payload decoding and client-side state reducers
  - Integration tests against the running backend URL
  - Browser end-to-end tests for join, ready, race, and results flows
- **Frameworks**:
  - The implementation team may choose any frontend stack and test framework
  - The only hard requirement is that the test environment can exercise a SignalR client connection and validate JSON payload handling
...
- **CI/CD Integration**:
  - Add a smoke test that joins a room, receives `TrackLoaded`, processes at least one `GameSnapshot`, and handles `RaceFinished`
  - Add an automated browser or headless-client test that verifies `ErrorMessage` handling for an invalid join scenario
- **Coverage Requirements**:
  - Every client -> server invocation SHALL be exercised by at least one automated test
  - Every server -> client event SHALL be decoded and rendered or processed by at least one automated test
...
The current backend favors same-origin browser connectivity. A generated frontend served from another origin must account for that deployment constraint explicitly by using a reverse proxy or a backend variant with explicit CORS support.
...
## 11. Usage Note for Student Teams

This specification is intended to be complete enough for a student team to implement a compatible frontend without access to backend source code.

Student teams should be given:

- this specification file
- the live backend origin and hub URL
- any desired brand assets or logo files, if visual customization matters

Student teams do not need:

- C# source code
- backend project files
- frontend reference implementation code

If maintainers later change the backend protocol, this specification must be updated and redistributed so student teams remain aligned with the live service.
