...
void CarParticleEffectManager::collision(size_t typeId, const MCVector3dF & contactPoint)
{
    if (m_car.physicsComponent().velocity().lengthFast() > 4.0f)
    {
        ...
        else if (typeId == MCObject::typeId("crate") || typeId == MCObject::typeId("vibeRacingBanner") || typeId == MCObject::typeId("grandstand") || typeId == MCObject::typeId("wall") || typeId == MCObject::typeId("wallLong") || typeId == MCObject::typeId("rock"))
        {
            for (size_t i = 0; i < particles; i++)
            {
                ParticleFactory::instance().doParticle(ParticleFactory::Sparkle,
